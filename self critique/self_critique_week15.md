...
- Exp B `best_by_eval` (unshaped reward, baseline): **20.0%** held-out (flat across 100 RL iterations).
- RL Qwen-1.5B long run (PrimeIntellect, 120 iters): best training EMA accuracy **42.3%** at iter 105; single-batch peak **75%** hit repeatedly; final iter 120 EMA 28.7%.
- **Shaped-reward run (Modal A10G, Apr 23):** held-out action-type accuracy **33.3%** at iter_5 and iter_10 (15 episodes each, same seed as Exp B baseline) — **+13.3 percentage points above Exp B**. Two independent checkpoints, same accuracy.
- Heuristic: **100%** (by construction).
- Training variants explored: v1 (temp=0.2, tok=512, 20 iters), v2 (temp=0.7, tok=512, 15 iters before timeout), v4 (temp=0.5, tok=1024, 9 iters before timeout). Only v1 completed held-out eval; the others revealed training-level patterns (see surprises below).
...
### Surprises
- **The central finding of the project is a negative result: the model learned to skip the REPL.** Across the 120-iteration long run, the `real_code` counter averaged 0.3 / 4 rollouts per batch — meaning roughly one in ten rollouts produced extractable Python. The rest hit the `wrapped_action_code` fallback, which just wraps a plain action string in `print(...)`. The policy discovered that the type-match reward fires ~25% of the time on any action guess, and writing a full retrieve→compute→decide program costs many tokens with high failure risk. REINFORCE followed the gradient downhill to the easy fallback. This is *exactly* the reward-hacking pattern §7 of the course notes teaches (the "avoid the letter e" example needed six reward iterations); we hit the same pathology on the first reward we tried.

- **The shaped-reward experiment we ran on the morning of the presentation actually worked.** Starting from Exp B's collapsed `best_by_eval` checkpoint and adding a tool-use bonus (+0.3 real code, +0.2 parsed stats) plus a fallback penalty (−0.2), the policy produced a held-out accuracy of 33.3% at both iter_5 and iter_10 — a 13-percentage-point improvement over Exp B's 20% flat line. Two separate checkpoints gave the same number, so it wasn't a single-seed fluke. This shows a collapsed-to-wrap policy *can* be rescued via reward shaping without retraining from scratch. For slide 11 this is the headline positive result of the whole project.

- **But the shaped-reward win exposed the next layer of reward hacking.** At higher token budget (1024 instead of 512) the policy *reliably* writes Python code (zero fallback wraps on 5 of 6 iterations in the v4 run) — but the code doesn't actually execute. Verbose rollout logs show extracted code like `for opp in opponents: vpip = stats.get(opp, {}).get('VPIP', 0)` where `opponents` and `stats` are undefined in the sandbox. The rollout still earns the +0.5 tool-use bonus because the reward measures "code extracted," not "code executed." Three layers of whack-a-mole: wrap → shaped-reward + short tokens → shaped-reward + long tokens that now writes syntactically-valid-but-non-executing code. A v5 experiment would gate the base_reward on `exec_ok=True`.

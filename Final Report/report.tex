...
fine-tune Qwen2.5-Coder-1.5B with LoRA and 4-bit NF4 quantization
using \texttt{trl.SFTTrainer}.
Key hyperparameters: LR $2\times10^{-4}$, cosine schedule with 10\%
warmup, batch $4\times4{=}16$, 2 epochs, max-length 4096.
At 2048 we truncated 46\% of examples, which motivated the longer
context window.
BC training takes $\approx$15 minutes on an H100.

\subsection{Phase 2 — REINFORCE}

Starting from the BC checkpoint, each RL iteration samples a batch of
rollouts at temperature 0.2 / top-p 0.9, executes each in the sandbox,
computes type-match reward, forms normalized clipped advantages
(\cref{eq:adv}), and updates with AdamW at LR $5\times10^{-6}$.
Checkpoints are saved every 5 iterations; best-by-held-out is selected
post-hoc.
...
\begin{figure}[t]
  \vskip 0.1in
  \begin{center}
    \centerline{\includegraphics[width=\columnwidth]{docs/results/poker_rl_long_simple_120iters_20260420/training_curves.png}}
...
\begin{lstlisting}[caption={Reward-hacking rollout: model emits text, not code.}]
...
# result: real_code=False, wrapped=True
# reward = 0.0  (wrong guess)
\end{lstlisting}
...
A successful post-shaping trace:

\begin{lstlisting}[caption={Successful REPL trace after reward shaping.}]
...
# Step 3 - decide
if agg > 2.0 and pot_odds < 0.35:
    print("call $6")
else:
    print("fold")

# result: real_code=True, exec_ok=True
# reward = 1.0  (type match: call == call)
\end{lstlisting}

\subsection{Summary Across Experiments}

\Cref{fig:heldout} shows held-out accuracy across all experiments with
explicit evaluation checkpoints.

\begin{figure}[t]
  \vskip 0.1in
  \begin{center}
    \centerline{\includegraphics[width=\columnwidth]{docs/results/analysis_all_experiments_20260423/heldout_eval_accuracy_over_time.png}}
    \caption{Held-out evaluation accuracy per checkpoint.
      Exp~A and~B plateau at 20–25\%; the shaped-reward run
      consistently reaches 33.3\%.}
    \label{fig:heldout}
  \end{center}
  \vskip -0.15in
\end{figure}
...
\begin{figure}[t]
  \vskip 0.1in
  \begin{center}
    \centerline{\includegraphics[width=\columnwidth]{docs/results/analysis_all_experiments_20260423/best_heldout_comparison.png}}

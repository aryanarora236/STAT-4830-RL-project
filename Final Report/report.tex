...
\Cref{fig:comparison} (Section~\ref{sec:results}) directly compares the best
...
\textbf{Range as a second component.}
The Slumbot bridge is built in parallel with a sister project called
Range, which provides the Slumbot client, wire-protocol parsing, and
leaderboard validation.  A Q-learning agent in Range currently ranks
\#3 worldwide on Slumbot's public leaderboard at +96.34~BB/100 over
5,000+ hands.  This establishes that Slumbot is a meaningful external
benchmark and that the integration is not just a toy API adapter.
...
The Range sister project's leaderboard result confirms Slumbot is a
